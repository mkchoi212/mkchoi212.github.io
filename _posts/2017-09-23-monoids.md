...
The above statement says that a binary operation of monoids takes two elements of type `T` and will always return `T`. Input type is same as output type. This property is very important as it will **allows us to chain operations indefinitely.**
...
To see how monoids can be used, we will create a library with power enough abstractions that will allows us to create some interesting ASCII art. Our library will mainly deal with creating geometric shapes.[^1] As we go thorugh the code, I will  demonstrate each example in *Haskell and then follow up with Swift.* 
...
We can first start by defining the `interesect` function which defines a new shape within the region of intersection.
